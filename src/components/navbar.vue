...
<template lang="pug">
  .navbar
    .navbar-header
      ...
      .logo
        | {{ navbarTitle }}
      .navbar-menu-pills(@click="toggleNavbar")
        i.fa.fa-bars
        .notification-badge(v-if="unreadsCount")
          template(v-if="fetching")
            i.fa.fa-spinner.fa-spin
          template(v-else)
            | {{ unreadsCount }}

    .navbar-menu(:class="navbarStatus" @click="toggleNavbar")
      .container
  ...
  export default {
    ...
    methods: {
      ...
      fetchUnreadsCount() {
        ...
        return this.$http.get('/cooperative/notifications/unreads_count', { loading: false })
          .then((response) => {
